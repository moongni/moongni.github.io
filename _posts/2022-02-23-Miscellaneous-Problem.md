...
`previous_items_value` 변수는 현재까지 탐색한 물건의 무게의 합이며 새로운 물건을 추가 할 수 있는지 확인한다.(if) `value_freeing_weight_for_item` (새 물건을 넣은 값)이 이전 탐색한 무게보다 더 큰 경우 `table`에 할당하고 더 적은 경우 이전 탐색한 무게를 할당한다.  


`table`은 아래와 같은 형식으로 작성되게 된다.  


|table | 0 파운드 | 1 파운드 | 2 파운드 | 3 파운드 | 4파운드 | 5파운드 | $\cdots$ | 75 파운드|
|---|---|---|---|---|---|---|---|---|
